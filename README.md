...
**BusinessPlanAI** — это современное Android-приложение, которое помогает пользователям быстро и просто создавать профессиональные бизнес-планы. Введите основную информацию о вашем бизнесе, и приложение автоматически сгенерирует структурированный и готовый к использованию бизнес-план с помощью модели ИИ.
...
| 🤖 **Генерация с помощью ИИ**    | Используете любую текстовую модель                             |
...
Приложение может использовать все текстовые модели, напрмиер такие как **Gemma-3-1b-it-qat** для генерации высококачественных бизнес-планов. Коммуникация происходит через защищённые HTTP-запросы, результаты автоматически форматируются и сохраняются в базу данных.
...
Несмотря на техническую сложность, **все функции экспорта работают отлично** и позволяют пользователям получать профессионально оформленные документы в нужном формате. Приложение использует продвинутый Markdown рендеринг с полной поддержкой стилизации.
...
## Инструкция как зпустить сервер и модель в LM-studio

| Нажмите на эту икноку        | Запустите локальный сервер          | Запустите ИИ модель         | Добавьте в приложение адресс вашего локального сервера и порта|
| ---------------------------- | ----------------------------------- |---------------------------- | ----------------------------------- |
|<img src="Instruction/1.jpg" width="250" alt="Превью 2" />|<img src="Instruction/2.jpg" width="800" alt="Превью 1" />|<img src="Instruction/3.jpg" width="700" alt="Превью 1" />|<img src="Instruction/4.jpg" width="700" alt="Превью 1" />|
